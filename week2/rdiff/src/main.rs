...
fn lcs(seq1: &Vec<String>, seq2: &Vec<String>) -> Grid {
    ...
    let len1 = seq1.len();
    let len2 = seq2.len();
    let mut result = Grid::new(len1+1, len2+1);
    for i in 0..len1+1 {
        result.set(i,0,0).unwrap();
    }
    for j in 0..len2+1 {
        result.set(0,j,0).unwrap();
    }
    for i in 0..len1 {
        for j in 0..len2 {
            if seq1[i]==seq2[j] {
                result.set(i+1,j+1,result.get(i,j).unwrap()+1).unwrap();
            }else{
                result.set(i+1,j+1,max(result.get(i+1,j).unwrap(),result.get(i,j+1).unwrap())).unwrap();
            }
        }
    }
    result
